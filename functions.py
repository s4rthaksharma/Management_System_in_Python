...
def add():
    try:
        ...
        b.execute('select * from data where s_adm="{}"'.format(adm))
        data=b.fetchall()
        #print(data)
        if data==[]:
            ...
            b.execute("Insert into data values('{}','{}','{}','{}')".format(name,adm,grade,address))
            a.commit()
...
def search():
    ...
    try:
        if choice==1:
            name=input('Enter student name:')
            b.execute('Select * from data where s_name="{}"'.format(name))
            data=b.fetchall()
            if data==[]:
                print('\t\t |******NO RECORDS FOUND******| ')
            else:
                print('\t\t |******RECORD FOUND******|')
                print(t.tabulate(data,headers=['Student name','Admission no.','Class','Address'],tablefmt='pretty'))
        elif choice==2:
            no=int(input('Enter admission number:'))
            b.execute('Select * from data where s_adm="{}"'.format(no))
            data=b.fetchone()
            print(data)
            if data==None:
                print('\t\t |******NO RECORDS FOUND******| ')
...
def delete():
    ...
    if choice==1:
        ...
        if data==[]:
            ...
            print('\n\t\t |******RECORD FOUND******|\n')
            print(t.tabulate(data,headers=['Student name','Admission no.','Class','Address'],tablefmt='grid'))
            if len(data)>1:
                adm=int(input('Enter admission number:'))
                b.execute("Select * from data where s_adm='{}'".format(adm))
                dd=b.fetchall()
                print(dd)
                print(t.tabulate(dd,headers=['Student name','Admission no.','Class','Address'],tablefmt='grid'))
                ...
                if opt.lower()=='yes':
                    ...
                    a3=dd[0][2]
                    a4=dd[0][3]
                    b.execute("Insert into deleted values('{}','{}','{}','{}')".format(a1,a2,a3,a4))
                    b.execute("Delete from data where s_name='{}'".format(name))
    ...
    if choice==2:
        ...
        if data==[]:
            ...
            if opt.lower()=='yes':
                ...
                a4=data[0][3]
                b.execute("Insert into deleted values('{}','{}','{}','{}')".format(a1,a2,a3,a4))
                b.execute("Delete from data where s_adm='{}'".format(adm))
                print('\n\t\t<<<<<<<<---RECORD HAS BEEN DELETED--->>>>>>>>\n')
                a.commit()
def update():
    print('\n\t\t-----------UPDATE MENU-----------')
            ...
            if opt.lower()=='yes':
                ...
                if opt1>=1 and opt1<=5:
                    ...
                    if opt1==1:
                        ...
                        b.execute("Update data set s_name='{}' where s_adm='{}'".format(name1,no))
                        
                    elif opt1==2:
                        adm=int(input('Enter new admission number:'))
                        b.execute("Update data set s_adm='{}' where s_adm='{}'".format(adm,no))
                        
                    elif opt1==3:
                        c1=input('Enter new class and section:')
                        b.execute("Update data set s_class='{}' where s_adm='{}'".format(c1,no))
                        
                    elif opt1==4:
                        add=input('Enter new address:')
                        b.execute("Update data set s_address='{}' where s_adm='{}'".format(add,no))
                    elif opt1==5:
                        ...
                        b.execute("Update data set s_name='{}',s_adm='{}',s_class='{}',s_address='{}' where s_adm='{}'".format(m,n,o,p,no))                
                    a.commit()
                    print('\n\t\t<<<<<<---RECORD UPDATED--->>>>>>\n')
                else:
...
def view():
    print('\n\t\t-----------VIEWING MENU-----------')
    print('1. View in alphabetical order (BY NAME)\n2. View in ascending order (ADMISSION NO.)')
    print('3. View in descending order (ADMISSION NO.)\n4. View in original order\n5. View in class order')
    choice=int(input('Enter choice:'))
    if choice==1:
    ...
    if choice==5:
            ...
            b.execute("Select * from data where s_class='{}'".format(grade))
            data=b.fetchall()
            if data==[]:
                print('\n\t\t*******NO RECORDS FOUND*******\n')
            else:
...
def restore():
    ...
    if choice==1:
        name=input('Enter student name:')
        b.execute("Select * from deleted where s_name='{}'".format(name))
        data=b.fetchall()
        if data==[]:
            print('\n\t*********NO RECORDS FOUND*********')
        else:
            for i in data:
                dd=[[i[0],i[1],i[2],i[3]]]
                print(t.tabulate(dd,headers=['Student name','Admission no.','Class & Section','Address'],tablefmt='grid'))
                ch=input('Type yes if you want this record to be restored:')
                if ch.lower()=='yes':
                    b.execute("Insert into data values('{}','{}','{}','{}')".format(i[0],i[1],i[2],i[3]))
                    b.execute("Delete from deleted where s_adm='{}'".format(i[1]))
                    a.commit()
        ...
        try:
            adm=int(input('Enter admission number:'))
            b.execute('Select * from deleted where s_adm="{}"'.format(adm))
            d=b.fetchall()
            if d==[]:
                print('\n\t*********NO RECORDS FOUND*********')
            else:
                ...
                a3=d[0][2]
                a4=d[0][3]
                print(t.tabulate(d,headers=['Student name','Admission no.','Class and Section','Address'],tablefmt='grid'))
                ch=input('Do you want to restore this record? (yes/no)')
                if ch.lower()=='yes':
                    b.execute("Insert into data values('{}','{}','{}','{}')".format(a1,a2,a3,a4))
                    b.execute("Delete from deleted where s_adm='{}'".format(adm))
                    a.commit()
                    print('\n\t--------DATA HAS BEEN RESTORED--------')
        except ValueError:
            print('\n\t\t------ADMISSION NO. MUST BE AN INTEGER------\n')
